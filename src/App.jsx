...
const QUESTION_ALIASES_RAW = {
  ...
  '들이마신 공기의 성분은 대부분 산소이고, 내쉰 공기의 성분은 대부분 이산화탄소이다.': '들숨의 성분은 대부분 산소이고, 날숨의 성분은 대부분 이산화탄소이다.',
  '혈액은 단순히 빨간 액체이다.': '혈액은 단순한 빨간 액체이다.',
  '심장이 피를 만든다.': '심장은 필요한 혈액량을 판단하여 박동수를 조절한다.',
  '오줌을 형성하는 곳은 방광이다.': '오줌을 형성하는 것은 방광이다.'
};
    ...
    experiments: [
      {
        name: '소화의 필요성',
        materials: [
          '비커 4개(침 용액 준비용 1개, 셀로판 튜브 담금용 3개)',
          '시험관 2개',
          '셀로판 튜브 3개',
          '스포이트 3~4개(용액별로 구분 사용)',
          '집게 1개',
          '온도계 1개',
          '핫플레이트 1개',
          '증류수',
          '포도당 용액 10mL',
          '녹말 용액 20mL(녹말 단독 10mL + 녹말+침 혼합용 10mL)',
          '침 용액 10mL(물 10mL를 입에 머금었다가 뱉어 준비)',
          '아이오딘-아이오딘화 칼륨 용액(반응 확인용 2~3방울)',
          '베네딕트 용액(시험관당 약 2mL)'
        ],
        steps: [
          '물 10mL를 2분 동안 입에 머금었다가 비커에 뱉어 침 용액 10mL를 준비한다.',
          '셀로판 튜브 3개를 준비하고, 각 튜브에 약 10mL씩 서로 다른 용액을 넣는다(포도당 용액 10mL / 녹말 용액 10mL / 녹말 5mL+침 5mL 혼합 용액).',
          '각 셀로판 튜브의 양 끝을 단단히 막은 뒤, 증류수 100mL가 담긴 비커 3개에 각각 1개씩 담근다.',
          '비커 속 증류수 온도를 37℃ 정도로 유지하며 20~30분간 관찰한다.',
          '녹말 튜브를 담가둔 비커의 바깥 용액을 작은 비커로 옮기고, 아이오딘-아이오딘화 칼륨 용액 2~3방울을 넣어 색 변화를 관찰한다.',
          '포도당 튜브를 담가둔 비커의 바깥 용액을 시험관으로 옮기고, 베네딕트 용액 약 2mL를 넣는다.',
          '시험관을 물중탕으로 가열하여 베네딕트 반응 결과를 관찰한다.',
          '녹말+침 혼합 용액 튜브를 담가둔 비커의 바깥 용액도 비커/시험관으로 나누어 옮긴 뒤, 아이오딘 반응과 베네딕트 반응을 각각 수행한다.'
        ]
      }
    ],
    reviewCards: [],
    ...
    experiments: [
      {
        ...
        materials: ['스마트폰 1대(심박수 측정 앱 설치)', '초시계 1개'],
        steps: [
          '의자에 편안히 앉아 2~3분간 휴식한 뒤 안정 시 심박수(BPM)를 3회 측정하고 평균값을 기록한다.',
          '1분 동안 제자리뛰기(또는 유사한 강도의 운동)를 실시한다.',
          '운동이 끝난 직후 즉시 심박수(BPM)를 측정하여 기록한다.',
          '운동 종료 3분 후 회복 상태의 심박수(BPM)를 다시 측정하여 기록한다.'
        ],
      ...
      },
      {
        ...
        materials: [
          ...
          '증류수 200mL(비커당 100mL)',
          'BTB 용액(비커당 3~4방울)',
          '수산화나트륨(NaOH) 용액(0.5mL씩 천천히 추가, 비커당 총 약 1mL 내외)'
        ],
        steps: [
          '두 비커에 증류수 100mL씩 넣고, BTB 용액을 각 비커에 3~4방울씩 넣는다.',
          '스포이트로 NaOH 용액을 0.5mL씩 천천히 추가하여 두 비커 색이 비슷한 파란색이 되도록 맞춘다(보통 비커당 총 약 1mL 내외).',
          '평상시 상태에서 한 비커에 빨대로 일정한 세기의 날숨을 불어넣고, 노란색으로 변할 때까지 시간을 측정한다.',
          '1분간 운동한 직후 다른 비커에 같은 방식으로 날숨을 불어넣고, 노란색으로 변할 때까지 시간을 측정한다.'
        ],
    ...
    experiments: [
      {
        ...
        materials: [
          ...
          '핫플레이트 1개',
          '증류수 300mL(500mL 비커용)',
          '가상 혈액 20~25mL(1% 녹말 수용액 10mL + 1% 포도당 수용액 10mL)',
          '아이오딘-아이오딘화 칼륨 용액',
          '베네딕트 용액'
        ],
        steps: [
          '셀로판 튜브를 물에 5분 정도 불려 부드럽게 만든 뒤 원통형으로 벌린다.',
          '튜브 한쪽 끝을 실(또는 고무줄)로 단단히 묶어 액체가 새지 않게 한다.',
          '스포이트로 튜브 안에 가상 혈액 약 20~25mL(녹말 10mL + 포도당 10mL 혼합)를 넣는다.',
          '공기를 최대한 제거한 뒤 반대쪽 끝도 묶어 주머니 형태를 만든다.',
          '완성된 주머니의 겉면을 물로 씻어낸다.',
          '500mL 비커에 증류수 300mL를 담고, 셀로판 튜브가 완전히 잠기도록 유리 막대에 매달아 넣은 뒤 20~30분간 둔다.',
          '시간이 지나면 셀로판 튜브를 건져내고, 바깥 비커의 용액을 채취한다.',
          '첫 번째 시험관에 바깥 용액 5mL를 넣고 아이오딘-아이오딘화 칼륨 용액 1~2방울을 떨어뜨려 반응을 확인한다.',
          '두 번째 시험관에 바깥 용액 5mL와 베네딕트 용액 1~2mL를 넣고 가열하여 반응을 확인한다.'
        ]
      }
    ],
    responseSections: {
